...
def fix_app_py():
    """Fix the app.py file."""
    app_py_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "fakeai", "app.py")
    ...
    if 'response_model=ChatCompletionRequest' in content:
        content = content.replace(
            'response_model=ChatCompletionRequest',
            'response_model=None'
        )
        print("Fixed: Changed response_model=ChatCompletionRequest to response_model=None in app.py")
    else:
...
def fix_service_py():
    ...
    service_py_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "fakeai", "fakeai_service.py")
    ...
    if stream_text in content and stream_replacement not in content:
        ...
        print("Fixed: Changed stream=False to stream=True in create_chat_completion_stream")
    else:
        print("No issue found with stream parameter in create_chat_completion_stream or already fixed")
