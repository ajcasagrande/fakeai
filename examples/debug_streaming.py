...
async def debug_streaming() -> None:
    ...
    try:
        ...
        if chunk_count > 0 and content_chunks == 0:
            ...
            print("This suggests the server is sending empty chunks or not generating content.")
        elif chunk_count == 0:
            print("\nISSUE DETECTED: No chunks received.")
            print("This suggests a server connection issue or streaming not working properly.")
    ...
    print("If both streaming and non-streaming requests have issues, the FakeAI server may not be running correctly.")
    print("If only streaming has issues, there might be a problem with the streaming implementation in FakeAI.")
    print("="*50)
