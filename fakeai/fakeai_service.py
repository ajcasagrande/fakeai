...
class FakeAIService:
    ...
    def __init__(self, config: AppConfig):
        ...
        self.fine_tuning_jobs: dict[str, FineTuningJob] = {}  # job_id -> FineTuningJob object
        self.fine_tuning_events: dict[str, list[FineTuningEvent]] = defaultdict(list)  # job_id -> list of events
        self.fine_tuning_checkpoints: dict[str, list[FineTuningCheckpoint]] = defaultdict(list)  # job_id -> list of checkpoints
        self.fine_tuning_tasks: dict[str, asyncio.Task] = {}  # job_id -> async task for background processing
    ...
    async def create_fine_tuning_job(self, request: FineTuningJobRequest) -> FineTuningJob:
        ...
        training_file = next((f for f in self.files if f.id == request.training_file), None)
        ...
        if request.validation_file:
            validation_file = next((f for f in self.files if f.id == request.validation_file), None)
        ...
        try:
            ...
            for step in range(1, total_steps + 1):
                ...
                if step % 10 == 0 or step in checkpoint_steps:
                    train_loss = 2.5 * (1 - step / total_steps) + random.uniform(0.0, 0.2)
                    valid_loss = train_loss + random.uniform(0.0, 0.3)
                    train_accuracy = 0.3 + (0.65 * step / total_steps) + random.uniform(0.0, 0.05)
                    ...
                    metrics = {
                        ...
                        "learning_rate": 0.0001 * job.hyperparameters.learning_rate_multiplier,
                    }
                ...
                if step in checkpoint_steps:
                    ...
                    if suffix:
                        checkpoint_model_name = f"ft:{job.model}:org-fakeai:{suffix}:{checkpoint_suffix}"
                    else:
                        checkpoint_model_name = f"ft:{job.model}:org-fakeai::{checkpoint_suffix}"
                    ...
                    checkpoint = FineTuningCheckpoint(
                        ...
                        step_number=step,
                        metrics={
                            "train_loss": round(2.5 * (1 - step / total_steps) + random.uniform(0.0, 0.2), 4),
                            "valid_loss": round(2.5 * (1 - step / total_steps) + random.uniform(0.0, 0.3), 4),
                            "train_accuracy": round(0.3 + (0.65 * step / total_steps) + random.uniform(0.0, 0.05), 4),
                        },
                    )
            ...
            training_file = next((f for f in self.files if f.id == job.training_file), None)
            if training_file:
                # Rough estimate: ~1 token per 4 bytes, multiplied by number of epochs
                estimated_tokens = (training_file.bytes // 4) * job.hyperparameters.n_epochs
